...
LOG_FILE="${STATE_DIR}/events-dashboard.log"
mkdir -p "$STATE_DIR"
...
docker compose exec -T \
    ...
    web \
    sh -lc 'mkdir -p /tmp/.cache/fontconfig && python manage.py events_dashboard --gui' \
    || fail "The events dashboard GUI exited with an error. See $LOG_FILE for details."
