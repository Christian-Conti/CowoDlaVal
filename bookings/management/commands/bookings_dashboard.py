...
class BookingWindow:
    def __init__(self, command, tk, ttk, messagebox, selected_date):
        ...
        toolbar.pack(fill="x")
        ttk.Button(toolbar, text=_("Previous day"), command=lambda: self.move_date(-1)).pack(side="left")
        self.date_value = tk.StringVar(value=selected_date.isoformat())
        ttk.Entry(toolbar, textvariable=self.date_value, width=12).pack(side="left", padx=6)
        ttk.Button(toolbar, text=_("Apply date"), command=self.refresh).pack(side="left")
        ttk.Button(toolbar, text=_("Next day"), command=lambda: self.move_date(1)).pack(side="left", padx=6)
        ...
        notes_frame = ttk.LabelFrame(self.root, text=_("Selected booking notes"), padding=8)
        ...
        ttk.Button(actions, text=_("Close"), command=self.root.destroy).pack(side="right")
        self.refresh()
    ...
    def selected_code(self, warn=True):
        ...
        if not selection:
            if warn:
                self.messagebox.showinfo(_("Bookings"), _("Select a booking first."))
            return None
        return selection[0]
    ...
    def refresh(self):
        ...
        confirmed = sum(
            booking.status == Booking.Status.CONFIRMED and booking.space.is_active for booking in bookings
        )
        active_spaces = Space.objects.filter(is_active=True).count()
        self.table.delete(*self.table.get_children())
        self.bookings = {booking.booking_code: booking for booking in bookings}
        ...
        self.summary.configure(
            ...
            % {"count": len(bookings), "available": max(active_spaces - confirmed, 0)}
        )
    ...
    def change_status(self, status):
        ...
        if code is None or not self.messagebox.askyesno(
            ...
            % {"code": code, "status": str(Booking.Status(status).label).lower()},
        ):
            return
        self._run_change(lambda: self.command._set_booking_status(code, status))

    def mark_paid(self):
        code = self.selected_code()
        if code is None or not self.messagebox.askyesno(
            _("Bookings"),
            _("Record the on-site payment for %(code)s?") % {"code": code},
        ):
            return
        self._run_change(lambda: self.command._mark_onsite_paid(code))

    def save_notes(self):
        code = self.selected_code()
        if code is None:
            return
        self._run_change(lambda: self.command._set_notes(code, self.notes.get("1.0", "end-1c")))
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        mode.add_argument("--cli", action="store_true", help=_("Force the terminal interface."))
        mode.add_argument("--gui", action="store_true", help=_("Force the desktop interface."))
        ...
        actions.add_argument("--cancel-booking", metavar="CODE", help=_("Cancel a booking and exit."))
        actions.add_argument("--confirm-booking", metavar="CODE", help=_("Confirm a booking and exit."))
        actions.add_argument("--mark-paid", metavar="CODE", help=_("Record an on-site payment and exit."))
        actions.add_argument("--set-notes", nargs=2, metavar=("CODE", "TEXT"), help=_("Replace notes and exit."))
    ...
    def handle(self, *args, **options):
        ...
        if options["from_date"] or options["to_date"]:
            if options["gui"]:
                raise CommandError(_("Date ranges are only available in the terminal interface."))
        ...
        requested_output = bool(options["selected_date"] or options["today"])
        if not options["cli"] and not requested_output and self._desktop_available():
    ...
    def _handle_action(self, options):
        if options["cancel_booking"]:
            booking = self._set_booking_status(options["cancel_booking"], Booking.Status.CANCELLED)
        elif options["confirm_booking"]:
            booking = self._set_booking_status(options["confirm_booking"], Booking.Status.CONFIRMED)
        elif options["mark_paid"]:
            booking = self._mark_onsite_paid(options["mark_paid"])
        elif options["set_notes"]:
            booking = self._set_notes(*options["set_notes"])
        else:
            return False
        self.stdout.write(self.style.SUCCESS(_("Updated %(code)s.") % {"code": booking.booking_code}))
        return True
    ...
    def _run_gui(self, selected, fallback):
        ...
        try:
            window = BookingWindow(self, tk, ttk, messagebox, selected)
    ...
    def _gui_error(self, error, fallback):
        message = _("The desktop interface could not start: %(error)s") % {"error": error}
        if not fallback:
    ...
    def _run_cli(self, selected):
        while True:
            ...
            try:
                choice = input(
                    _("[p]revious [n]ext [j]ump [r]ange [c]ancel con[f]irm [m]ark paid [e]dit notes [q]uit: ")
                ).strip().lower()
                if choice == "q":
                    return
                if choice == "p":
    ...
    def _interactive_change(self, choice):
        ...
        try:
            if choice == "c":
                booking = self._set_booking_status(code, Booking.Status.CANCELLED)
            elif choice == "f":
                booking = self._set_booking_status(code, Booking.Status.CONFIRMED)
            elif choice == "m":
                booking = self._mark_onsite_paid(code)
            else:
                booking = self._set_notes(code, input(_("New notes (empty clears them): ")))
        except CommandError as exc:
    ...
    def _bookings_for_date(self, selected):
        return Booking.objects.filter(date=selected).select_related("space", "user").order_by("space__name", "status")
    ...
    def _render_date(self, selected: date) -> None:
        ...
        cancelled = [booking for booking in bookings if booking.status == Booking.Status.CANCELLED]
        if cancelled:
            self.stdout.write(self._style(_("Cancelled bookings"), "1"))
            for booking in cancelled:
                self.stdout.write(f"  {booking.space}")
                self._render_booking(booking, indent="    ")
    ...
    def _render_booking(self, booking, indent="  "):
        ...
        if booking.payment_due:
            indicator = self._style(indicator, "1;31")
        elif booking.payment_status == Booking.PaymentStatus.PAID:
            indicator = self._style(indicator, "1;32")
        else:
            indicator = self._style(indicator, "1;33")
        self.stdout.write(f"{indent}{indicator}")
        if booking.notes:
            self.stdout.write(f"{indent}{_('Notes')}: {' '.join(booking.notes.split())}")
    ...
    def _set_booking_status(self, code, status):
        ...
        if status == Booking.Status.CONFIRMED and Booking.objects.filter(
            space_id=booking.space_id,
            date=booking.date,
            status=Booking.Status.CONFIRMED,
        ).exclude(pk=booking.pk).exists():
            raise CommandError(
                _("%(space)s already has a confirmed booking on %(date)s.")
                % {"space": booking.space, "date": booking.date.isoformat()}
            )
        booking.status = status
        try:
            booking.save(update_fields=["status", "updated_at"])
        except IntegrityError as exc:
            raise CommandError(_("The booking conflicts with another confirmed booking.")) from exc
        return booking
    ...
    def _mark_onsite_paid(self, code):
        booking = self._locked_booking(code)
        if booking.status != Booking.Status.CONFIRMED:
            raise CommandError(_("Only confirmed bookings can be marked paid."))
        if booking.payment_method != Booking.PaymentMethod.ONSITE:
            raise CommandError(_("Only payments made on site can be recorded manually."))
    ...
    def _set_notes(self, code, notes):
        try:
            ...
            raise CommandError(_("Booking %(code)s does not exist.") % {"code": code}) from exc
        booking.notes = notes
    ...
    def _locked_booking(self, code):
        try:
            return Booking.objects.select_for_update().select_related("space").get(booking_code=code)
        except Booking.DoesNotExist as exc:
            raise CommandError(_("Booking %(code)s does not exist.") % {"code": code}) from exc
